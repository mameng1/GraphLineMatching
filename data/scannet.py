...
train_anno_path = cfg.scannet.TRAIN_KEY_POINT_DIR
train_img_path = cfg.scannet.TRAIN_IMAGE_DIR
test_img_path = cfg.scannet.TEST_IMAGE_DIR
test_anno_path = cfg.scannet.TEST_KEY_POINT_DIR
...
class Scannet:
    ...
    def get_pair(self, idx,cls=None, shuffle=True):
        ...
        for anno_name,img_name in zip(annos_list,img_list):
            anno_dict = self.__get_anno_dict(anno_name, img_name,orig_idx)
            if(shuffle):
        ...
        if(img_list[0].find("/media/mameng") !=-1):
            valid_rowsid=row_list
            valid_colsid=col_list
    ...
    def __get_anno_dict(self, anno_name,img_name,orig_idx):
        """
        ...
        with open(str(anno_file.absolute()), "r") as anno:
            ...
            for keypoint in all_keypts:
                ...
                #draw.line((keypoint[1],keypoint[2],keypoint[3],keypoint[4]),fill=(255,255,255),width=7)
        #obj.show()
        anno_dict = dict()
        anno_dict['image'] = obj
