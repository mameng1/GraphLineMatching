...
template <typename T>
__global__ void LinePoolAlignForward(
    const int nthreads,
    const T* bottom_data,
...
std::tuple<at::Tensor, at::Tensor, at::Tensor> LinePoolAlign_forward_cuda(
                                  ...
                                  const int pooled_width)
{
    ...
    AT_DISPATCH_FLOATING_TYPES(input.type(), "LinePoolAlign_forward", [&] {
        LinePoolAlignForward<scalar_t><<<grid, block, 0, stream>>>(
        output_size,
...
template <typename T>
__global__ void LinePoolAlignBackward(
            const int nthreads,
            const T* top_diff,
...
at::Tensor LinePoolAlign_backward_cuda(const at::Tensor& grad,
  ...
  AT_DISPATCH_FLOATING_TYPES(grad.type(), "LinePoolAlign_backward", [&] {
    LinePoolAlignBackward<scalar_t><<<grid, block, 0, stream>>>(
         grad.numel(),
