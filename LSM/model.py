...
from LSM.gconv import Siamese_Gconv
from LSM.affinity_layer import Affinity
#from extension.line_pooling.line_pooling import LinePooling
...
class Net(CNN):
    def __init__(self,output_size,scales):
        ...
        self.bi_stochastic = Sinkhorn(max_iter=cfg.LSM.BS_ITER_NUM, epsilon=cfg.LSM.BS_EPSILON)
        self.voting_layer = Voting(alpha=cfg.LSM.VOTING_ALPHA)

        self.l2norm = nn.LocalResponseNorm(cfg.LSM.FEATURE_CHANNEL * 2, alpha=cfg.LSM.FEATURE_CHANNEL * 2, beta=0.5, k=0)
        self.gnn_layer = cfg.LSM.GNN_LAYER
        self.feature_channel=cfg.LSM.FEATURE_CHANNEL
        gauss_k = cv2.getGaussianKernel(output_size[0], 5.5, cv2.CV_32F)
        ...
        self.pos_layer=PosFeatureLayer(4,cfg.LSM.FEATURE_CHANNEL * 2)

        for i in range(self.gnn_layer):
            if i == 0:
                gnn_layer = Siamese_Gconv(cfg.LSM.FEATURE_CHANNEL * 2, cfg.LSM.GNN_FEAT)
                adjacA = GraphAttentionLayer(cfg.LSM.FEATURE_CHANNEL * 2, 16, 0.8, 0.2,3,i)

            else:
                gnn_layer = Siamese_Gconv(cfg.LSM.GNN_FEAT, cfg.LSM.GNN_FEAT)
                adjacA = GraphAttentionLayer(cfg.LSM.GNN_FEAT, 16, 0.8, 0.2,3,i)

            self.add_module('adjacA_{}'.format(i), adjacA)
            #self.add_module('adjacB_{}'.format(i), adjacB)
            self.add_module('gnn_layer_{}'.format(i), gnn_layer)
            self.add_module('affinity_{}'.format(i), Affinity(cfg.LSM.GNN_FEAT))
            if i == self.gnn_layer - 2:  # only second last layer will have cross-graph module
                self.add_module('cross_graph_{}'.format(i), nn.Linear(cfg.LSM.GNN_FEAT * 2, cfg.LSM.GNN_FEAT))
    ...
    def forward(self, src, tgt, P_src, P_tgt, ns_src, ns_tgt,train_stage=True,perm_mat=None,score_thresh=None):
        ...
        for i in range(self.gnn_layer):
            ...
            if i == self.gnn_layer - 2:
                ...
                emb2 = emb2_new
        if(train_stage):
            return s, None,match_emb1,match_emb2,match_edgeemb1,match_edgeemb2,perm_mat,ns_src,ns_tgt
        else:
            return s,None,match_emb1,match_emb2,match_edgeemb1,match_edgeemb2,indeces1,indeces2,ns_src,ns_tgt
    def line_feature_align(self,raw_feature, P, ns_t, layer_idx,device=None):
        ...
        if(self.gauss_k is not None):
            ...
            rois_feature=torch.sum(rois_feature,dim=2)
        else:
    ...
    def filterOutlier(self, low_emb1,low_emb2,high_emb1,high_emb2,ns_src, ns_tgt,score_thresh):
        ...
        for b in range(batch_size):
            ...
            row_max_i=torch.where((lrow_max_v>score_thresh))[0]#torch.where((lrow_max_v>score_thresh)&(hrow_max_v>(2*score_thresh/3)))[0]
            col_max_i=torch.where((lcol_max_v>score_thresh))[0]#torch.where((lcol_max_v>score_thresh)&(hcol_max_v>(2*score_thresh/3)))[0]
    ...
    def scatter_tensor(self,indeces1,indeces2,ns_src,ns_tgt,perm,ns_src_orig,ns_tgt_orig):
        batch_size=len(indeces1)
        max_rownum=torch.max(ns_src_orig)+1
        max_colnum=torch.max(ns_tgt_orig)+1
        new_perm=torch.zeros((batch_size,max_rownum,max_colnum),device=ns_src.device)
        for b in range(batch_size):
            batch_perm=perm[b]
            slice_perm=batch_perm[:ns_src[b]+1,:ns_tgt[b]+1]

            batch_indeces1=indeces1[b]
            batch_indeces2=indeces2[b]

            b_indecesn1=torch.zeros((ns_src[b]+1), dtype=torch.int32,device=ns_src.device)
            b_indecesn1[:ns_src[b]]=batch_indeces1
            b_indecesn1[ns_src[b]]=ns_src_orig[b]

            b_indecesn2 = torch.zeros((ns_tgt[b] + 1), dtype=torch.int32,device=ns_src.device)
            b_indecesn2[:ns_tgt[b]] = batch_indeces2
            b_indecesn2[ns_tgt[b]] = ns_tgt_orig[b]

            new_perm[b, ns_src[b], :] = 1
            new_perm[b, :,ns_tgt[b]] = 1
            for ridx,r_val in enumerate(b_indecesn1):
                for cidx,c_val in enumerate(b_indecesn2):
                    new_perm[b,r_val,c_val]=slice_perm[ridx,cidx]
            return new_perm